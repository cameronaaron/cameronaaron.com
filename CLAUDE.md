...
Lighthouse thresholds: **1.0 on every category, both desktop and mobile**, with `numberOfRuns: 1` in both configs (enforced by `lighthouserc.json`, `lighthouserc.mobile.json`, and `src/performance-regression-contract.test.ts`). This became viable when the Speed Index bottleneck was fixed at the source — `IntroCurtain` now dismisses via a pure CSS animation instead of waiting on hydration. See `ENGINEERING-STANDARDS.md` §4.7 for the history (including a measured-and-rejected code-splitting experiment) and for a known local-vs-CI Lighthouse false positive (`bf-cache`) — trust CI's numbers over a local `npm run test:performance:*` run.
