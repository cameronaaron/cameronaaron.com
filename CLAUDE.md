...
Lighthouse performance threshold: **0.90 desktop / 1.0 mobile**, accessibility/best-practices/SEO pinned to 1.0 on both (enforced by `lighthouserc.json`, `lighthouserc.mobile.json`, and `src/performance-regression-contract.test.ts`). See `ENGINEERING-STANDARDS.md` §4.7 for the data behind why desktop isn't 1.0 (two rounds of threshold/numberOfRuns tuning, backed by observed medians) and for a known local-vs-CI Lighthouse false positive (`bf-cache`) — trust CI's numbers over a local `npm run test:performance:*` run.
